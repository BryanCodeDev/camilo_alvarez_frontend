      ...
      <div className="min-h-screen bg-primary-900 pt-20">
        <div className="container-custom py-8">
          <motion.div
            ...
            className="mb-8"
          >
            <h1 className="font-display font-bold text-3xl sm:text-4xl text-white">Carrito de compras</h1>
            <p className="text-white mt-2">{itemCount} {itemCount === 1 ? 'producto' : 'productos'} en tu carrito</p>
          </motion.div>
          ...
          {loading ? (
            ...
            >
              <svg className="w-24 h-24 text-primary-600 mb-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M16 11V7a2 2 0 00-2-2H5a2 2 0 00-2 2v4m0 0h14m-5 5v1a2 2 0 01-2 2H7a2 2 0 01-2-2v-1m14-5l-3.5-3.5a2 2 0 00-2.828 0L8 11m7 0l-3.5 3.5a2 2 0 01-2.828 0"/></svg>
              <h2 className="font-display font-bold text-2xl text-white mb-2">Tu carrito está vacío</h2>
              <p className="text-white mb-8 max-w-md">Aún no has agregado ningún producto. ¡Empieza a explorar nuestra tienda!</p>
                            ...
                            >
                              <td className="px-6 py-4">
                                <Link to={`/producto/${item.slug}`} className="flex items-center gap-4">
                                  ...
                                  <div className="min-w-0">
                                    ...
                                    <p className="text-white text-sm">{item.sku}</p>
                                    {item.discountPrice && item.price > item.discountPrice && (
                                      <p className="text-green-500 text-sm">Ahorras {formatPrice((item.price - item.discountPrice) * item.quantity)}</p>
                                    )}
                                  </div>
                                </Link>
                              </td>
                              <td className="px-6 py-4 text-center hidden sm:table-cell">
                                <div className="flex flex-col items-center gap-1">
                                  ...
                                  {item.discountPrice && item.price > item.discountPrice && (
                                    <span className="text-white line-through text-sm">{formatPrice(item.price)}</span>
                                  )}
                                </div>
                              </td>
                ...
                <div className="lg:col-span-1">
                  ...
                  >
                    ...
                    <div className="space-y-3 mb-6">
                      ...
                      <div className="flex justify-between text-sm text-white border-t border-dark-border pt-3">
                        <span>Envío</span>
